(() => {
  function PAGE_WORKER() {
    ...
    const VERSION = 'player-codex-global-aruba-v6.2';
    ...
    async function backToList(type){
      if(currentType()===type)return;
      if(typeof window.UTIL_handleLeftMenuIconClick==='function')window.UTIL_handleLeftMenuIconClick(true);
      else {
        const back=[...document.querySelectorAll('#back-button')].find(visible)||document.getElementById('back-button');
        if(back&&visible(back))click(back);
        else throw new Error('Comando indietro non disponibile · v6.2');
      }
      for(let i=0;i<110;i++){await sleep(120);if(currentType()===type)return;}
      throw new Error('Lista giocatori non ripristinata · v6.2');
    }
  ...
  try{const script=document.createElement('script');script.textContent=`(${PAGE_WORKER.toString()})();`;(document.documentElement||document.head).appendChild(script);script.remove();if(typeof completion==='function')completion('IMC Player Codex Global Aruba/MySQL v6.2 avviato');}
