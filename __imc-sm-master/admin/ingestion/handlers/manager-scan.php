...
function manager_scan_handle(mysqli $db,array $body,string $action): never {
    ...
    try{$db->begin_transaction();
        ...
        $after=manager_scan_previous($db,$gw,$entityType);$events=manager_scan_events($before,$after,$imc,$scan);$externalCount=0;
        foreach($after as $assignment){$managerKey=(int)$assignment['sm_manager_id'];if(!isset($imc[$managerKey]))$externalCount++;}
        $stats=manager_scan_json(['scan_type'=>$scan,'entities'=>count($normalized),'assigned'=>count($after),'external_managers'=>$externalCount,'events'=>count($events)]);
        ...
        ingestion_reply(['ok'=>true,'change_status'=>$events?'CHANGE':'NO_CHANGE','events'=>$events,'run_id'=>$runId,'capture_id'=>$captureId,'received_count'=>count($normalized),'assigned_count'=>count($after),'external_manager_count'=>$externalCount]);
