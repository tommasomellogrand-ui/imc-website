...
function smm_handle_ingestion():never{
    ...
    if(in_array($type,['results','schedule','match_report','player_codex','player_global','transfers','manager_scan'],true)){
        if($type==='player_global'){
            require_once __DIR__.'/ingestion/handlers/player-global.php';
            player_global_handle(smm_db(),$body,(string)($body['action']??''));
        }
...
function smm_ingestion_auth(array $payload=[]):void{
    ...
    $playerCodexAuthorized=$token!==''&&in_array($importerType,['player_codex','player_global'],true)&&hash_equals(SMM_MATCH_REPORT_INGESTION_TOKEN,$token);
