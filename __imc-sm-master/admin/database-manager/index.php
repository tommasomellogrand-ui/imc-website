...
function dbm_cleanup_empty_non_imc_tables(array $payload): array {
    $target = trim((string)($payload['target'] ?? ''));
    if (!in_array($target, ['core','gold','custom'], true)) throw new InvalidArgumentException('Cleanup target must be core, gold or custom.');
    if (trim((string)($payload['confirm'] ?? '')) !== 'AUTHORIZED_EMPTY_NON_IMC_CLEANUP') throw new InvalidArgumentException('Explicit cleanup confirmation required.');
    [$database, $db] = dbm_storage($target);
    $result = $db->query("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_TYPE='BASE TABLE'");
    $candidates=[];
    while ($row=$result->fetch_assoc()) {
        $table=(string)$row['TABLE_NAME'];
        if (stripos($table,'IMC') !== false) continue;
        $candidates[]=$table;
    }
    $result->free(); sort($candidates,SORT_STRING);
    $dropped=[]; $skipped=[]; $errors=[];
    ...
    $record=['action'=>'cleanup_empty_non_imc_tables','target'=>$target,'database'=>$database,'candidate_count'=>count($candidates),'dropped_count'=>count($dropped),'skipped_nonempty_count'=>count($skipped),'error_count'=>count($errors),'status'=>$errors===[]?'success':'partial'];
    dbm_audit($record);
    return ['ok'=>$errors===[]]+$record+['dropped'=>$dropped,'skipped_nonempty'=>$skipped,'errors'=>$errors];
}

function dbm_cleanup_gold_legacy_gw_tables(array $payload): array {
    if (trim((string)($payload['confirm'] ?? '')) !== 'AUTHORIZED_GOLD_LEGACY_GW_CLEANUP') throw new InvalidArgumentException('Explicit cleanup confirmation required.');
    $tables = ['gw_competition_editions','gw_competitions','gw_countries','gw_fixture_results','gw_fixtures','gw_game_worlds','gw_import_runs','gw_match_commentary','gw_match_events','gw_match_lineup_players','gw_match_lineups','gw_match_reports','gw_match_tactics_snapshots','gw_match_team_stats','gw_national_teams','gw_player_db_update_batches','gw_player_profile_snapshots','gw_players','gw_seasons','gw_sm_user_assignments','gw_sm_users','gw_source_captures','gw_source_dom_nodes','gw_source_fragments','gw_source_page_types','gw_tactic_snapshots','gw_transfer_summary_snapshots','gw_transfers','gw_world_clubs'];
    ...
    foreach($tables as $table) {
        ...
        try { $db->query("DROP TABLE ".$quoted); $dropped[]=$table; }
        catch(Throwable $e) { $errors[]=['table'=>$table,'error'=>$e->getMessage()]; }
    }
    $record=['action'=>'cleanup_gold_legacy_gw_tables','target'=>'gold','database'=>$database,'requested_count'=>count($tables),'dropped_count'=>count($dropped),'missing_count'=>count($missing),'error_count'=>count($errors),'status'=>$errors===[]?'success':'partial'];
    dbm_audit($record);
...
function dbm_owner_selected_table_cleanup(array $payload): array {
    if (trim((string)($payload['confirm'] ?? '')) !== 'AUTHORIZED_OWNER_SELECTED_TABLE_CLEANUP') throw new InvalidArgumentException('Explicit owner confirmation required.');
    $target=trim((string)($payload['target'] ?? ''));
    $sets=[
      'gold'=>['GW002_IMC SM Player Stats','GW002_results','GW002_schedule','GW002_transfers','GW003_IMC SM Player Stats','GW003_results','GW003_schedule','GW003_transfers','GW007_IMC SM Player Stats','GW007_results','GW007_transfers','GW008_IMC SM Player Stats','GW008_results','GW008_schedule','GW008_transfers'],
      'custom'=>['GW001_player_codex','GW001_results','GW001_schedule','GW001_transfers','GW005_IMC Match Report','GW004_IMC SM Player Stats','GW004_results','GW004_transfers','GW005_IMC SM Player Stats','GW005_results','GW005_transfers','GW006_IMC SM Player Stats','GW006_results','GW006_transfers','GW009_IMC SM Player Stats','GW009_player_codex','GW009_results','GW009_schedule','GW009_transfers','GW010_IMC SM Player Stats']
    ];
    if(!isset($sets[$target])) throw new InvalidArgumentException('Target must be gold or custom.');
    [$database,$db]=dbm_storage($target); $dropped=[]; $missing=[]; $errors=[];
    foreach($sets[$target] as $table) {
      $quoted=chr(96).str_replace(chr(96),chr(96).chr(96),$table).chr(96);
    ...
    $record=['action'=>'owner_selected_table_cleanup','target'=>$target,'database'=>$database,'requested_count'=>count($sets[$target]),'dropped_count'=>count($dropped),'missing_count'=>count($missing),'error_count'=>count($errors),'status'=>$errors===[]?'success':'partial'];
    dbm_audit($record);
    return ['ok'=>$errors===[]]+$record+['dropped'=>$dropped,'missing'=>$missing,'errors'=>$errors];
}
