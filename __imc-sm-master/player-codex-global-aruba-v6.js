(() => {
  function PAGE_WORKER() {
    ...
    const VERSION = 'player-codex-global-aruba-v6.1';
    ...
    async function backToList(type){
      ...
      else if(typeof window.UTIL_handleLeftMenuIconClick==='function')window.UTIL_handleLeftMenuIconClick(true);
      else throw new Error('Comando indietro non disponibile');
      for(let i=0;i<110;i++){await sleep(120);if(currentType()===type)return;}
      throw new Error('Lista giocatori non ripristinata');
    }
  ...
  try{const script=document.createElement('script');script.textContent=`(${PAGE_WORKER.toString()})();`;(document.documentElement||document.head).appendChild(script);script.remove();if(typeof completion==='function')completion('IMC Player Codex Global Aruba/MySQL v6.1 avviato');}
