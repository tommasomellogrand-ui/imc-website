(()=>{
  ...
  function watchCompletion(){const count=document.getElementById('count');if(!count)return;let scheduled=false;const check=()=>{if(!scheduled&&/24\s*\/\s*24\s*LOCKED/i.test(count.textContent)){scheduled=true;setTimeout(showFinal,3200);}};new MutationObserver(check).observe(count,{childList:true,subtree:true,characterData:true});check();}
