(() => {
  ...
  function card(row,dataset){
    ...
    return `<article class="team-hub-card">
      <div class="team-hub-logo">${/^https?:\/\//i.test(img)?`<img src="${esc(img)}" alt="${esc(name)}" loading="lazy" onerror="this.remove()">`:esc(initials(name))}</div>
