-- SINGLE-LEAGUE FINAL DDL · CHECKPOINT 08
-- Derived exclusively from the installed GOLD schema (CHECKPOINT 07), excluding structural Country/National Team hierarchy
...
-- Phase 1: create all 53 single-league tables, primary keys, unique keys and indexes.
...
CREATE TABLE `gw_sm_user_assignments` (
  ...
  `capture_id` BIGINT UNSIGNED NOT NULL,
  `created_at` DATETIME(6) NOT NULL,
  `entity_type` ENUM('CLUB') NOT NULL,
  `game_world_id` VARCHAR(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  `gw_season_row_id` BIGINT UNSIGNED NULL,
  `imc_season` INT UNSIGNED NULL,
