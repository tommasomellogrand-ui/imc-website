(() => {
  ...
  function parseCompetitionHash(hash){
    ...
    try{
      ...
      const key=String(p?.competition_key||'').trim()||null;
      const smAction=String(p?.sm_action||'').trim()||null;
      const smDivision=String(p?.sm_division||'').trim()||null;
      if(!group||(!key&&!smAction))return null;
      return{group,competition_key:key,sm_action:smAction,sm_division:smDivision};
    }catch(_){return null;}
  }
  const competitionHash=i=>`#/competition/${encodeURIComponent(JSON.stringify({group:i.group,competition_key:i.competition_key,sm_action:i.sm_action,sm_division:i.sm_division}))}`;
  ...
  function render(i,active='overview'){const view=APP?.querySelector('.view');if(!view||!i?.competition_key)return false;const token=++requestToken;view.innerHTML=pageMarkup(i,active);window.scrollTo({top:0,behavior:'instant'});if(active==='overview')loadOverview(i,token);if(active==='results')loadResults(i,token);if(active==='schedule')loadSchedule(i,token);if(active==='stats')loadStats(i,token);if(active==='trophy')loadTrophy(i,token);return true;}

  async function gatewayRows(repository,filters={}){
    const u=new URL(GATEWAY);
    u.searchParams.set('game_world_id',WORLD);
    u.searchParams.set('action','read');
    u.searchParams.set('repository',repository);
    u.searchParams.set('limit','1000');
    Object.entries(filters).forEach(([k,v])=>{if(v!==null&&v!==undefined&&v!=='')u.searchParams.set(`filter_${k}`,String(v));});
    const c=new AbortController();const t=setTimeout(()=>c.abort(),12000);let r;
    try{r=await fetch(u.toString(),{cache:'no-store',headers:{Accept:'application/json'},signal:c.signal});}finally{clearTimeout(t)}
    let p;try{p=await r.json()}catch(_){throw new Error(`Gateway response non JSON (${r.status})`)}
  ...
  async function openCompetition(hash,replace=false){
    const parsed=parseCompetitionHash(hash);if(!parsed)return false;
    const identity=await resolveCompetitionKey(parsed);
    if(!identity)return false;
    const canonical=competitionHash(identity);
    const url=`${location.pathname}${location.search}${canonical}`;
    if(replace)history.replaceState({gw001Competition:true},'',url);
    else history.pushState({gw001Competition:true},'',url);
  ...
  document.addEventListener('click',async event=>{
    ...
    if(link){
      ...
      if(n>=0){
        ...
        if(parseCompetitionHash(hash)){
          event.preventDefault();
          event.stopImmediatePropagation();
          try{await openCompetition(hash,false);}catch(_){return;}
          return;
        }
      }
    }
  ...
  if(pending){window.__GW001_PENDING_COMPETITION_HASH__=null;openCompetition(pending,true).catch(()=>{});}
})();