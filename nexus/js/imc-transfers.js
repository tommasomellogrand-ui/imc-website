...
.imct-section{display:flex;align-items:center;justify-content:space-between;margin:16px 0 7px}.imct-section strong{font-size:11px;font-weight:1000}.imct-section span{color:#8a9097;font-size:8px;font-weight:900}.imct-list{overflow:hidden;border:1px solid #e5e8eb;border-radius:16px}.imct-row{cursor:pointer;display:grid;grid-template-columns:34px minmax(0,1fr) auto;gap:7px;align-items:center;min-height:64px;padding:7px 8px;border-top:1px solid #eceef0}.imct-row:first-child{border-top:0}.imct-row .imct-player{width:32px;height:32px;border-radius:50%;background:#eef1f3;color:#505962;font-size:10px}.imct-copy{min-width:0}.imct-copy h3{margin:0 0 3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px;font-weight:1000}.imct-side{text-align:right;white-space:nowrap}.imct-side strong{display:block;color:#168d43;font-size:13px;font-weight:1000}.imct-side time{display:block;margin-top:4px;color:#979ca3;font-size:7px;font-weight:850}.imct-empty{padding:22px 12px;color:#7d838b;text-align:center;font-size:10px;font-weight:800}.imct-more{width:100%;height:43px;margin-top:9px;border:1px solid #dce2e6;border-radius:12px;background:#fff;color:#168d43;font:inherit;font-size:9px;font-weight:1000}.imct-more[hidden]{display:none}
...
async function transfers(id,force){
  ...
  while(true){
    const res=await fetch("/nexus/public/data.php?world="+encodeURIComponent(id)+"&resource=transfers&limit=5000&offset="+offset,{cache:"no-store",credentials:"same-origin"});
    if(!res.ok)throw new Error("Transfers Aruba HTTP "+res.status);
    const d=await res.json();if(!d||!d.ok)throw new Error((d&&d.error)||"Transfers Aruba unavailable");
    const page=Array.isArray(d.rows)?d.rows:[];out.push(...page);
    if(page.length<5000)break;offset+=5000;
  }
  const rows=out.map(x=>Object.assign({},x,{transfer_id:x.imc_transfer_number,transfer_code:x.imc_transfer_number,transfer_date_text:x.normalized_transfer_date||"",imc_season:x.imc_season||x.normalized_season||null})).sort((a,b)=>String(b.transfer_date_text||"").localeCompare(String(a.transfer_date_text||""))||Number(b.imc_transfer_number||0)-Number(a.imc_transfer_number||0));
...
function rowMarkup(x){return `<article class="imct-row" data-tid="${esc(x.transfer_id)}">${media(x)}<div class="imct-copy"><h3>${esc(x.display_name||("Player "+x.player_id))}</h3>${route(x)}</div><div class="imct-side"><strong>${esc(x.amount_text||"—")}</strong><time>${esc(x.transfer_date_text||"—")}${x.season_number?` · S${esc(x.season_number)}`:""}</time></div></article>`;}
