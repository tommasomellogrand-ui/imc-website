...
function getWorldMeta(worldId){
  ...
  function officialLeagueMatchday(){
    return null;
  }
...
<div class="nx-codex-filter-actions-b51"><button type="button" id="playerCodexResetB51" class="nx-codex-reset-b51">Reset</button><button type="button" id="playerCodexApplyB51" class="nx-codex-apply-b51">Applica filtri</button></div>
  ...
  async function loadDivisionData(){
    ...
    if(!target||!state.client)return;
    if(state.divisionTab==="results"||state.divisionTab==="schedule"){
    ...
    try{
      ...
      // Core 54: Results/Schedule are rendered only by dedicated per-GW repositories.
      return;
    }catch(error){
  ...
  async function loadCompetitionData(){
    ...
    if(!target||!state.client)return;
    if(state.competitionTab==="results"||state.competitionTab==="schedule"){
      target.innerHTML='<div class="nx-loading">Caricamento dati…</div>';
      return;
    }
...
/* CORE 54 · Repository-only Results + Schedule */
(function(){
  "use strict";
