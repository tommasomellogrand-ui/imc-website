...
try {
 ...
 THEN CAST(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(market_value),'€',''),'£',''),'
    if($resource==='stats')respond(nexus_stats($pdo,$world,$coreDb));
    if($resource==='competition_reports')respond(nexus_competition_reports($pdo,$coreDb,$world));
    if($resource==='manager_profile')respond(nexus_manager_profile($pdo,$coreDb,$world));
    if($resource==='team_profile')respond(nexus_team_profile($pdo,$coreDb,$world));
...
,''),'M',''),'K',''),',','.') AS DECIMAL(18,3))*CASE WHEN UPPER(TRIM(market_value)) LIKE '%M' THEN 1000000 WHEN UPPER(TRIM(market_value)) LIKE '%K' THEN 1000 ELSE 1 END ELSE NULL END)";
        ...
        $sort=$orders[$_GET['sort']??'rating']??$orders['rating'];
        ...
        foreach($rows as &$row){
            $row['image_url']=($row['player_core']['image_url']??null)?:$row['image_url'];
            foreach(['rating_history','transfer_history'] as $field)if(array_key_exists($field,$row))$row[$field]=$row[$field]===null?[]:(json_decode($row[$field],true)?:[]);
        }unset($row);
        $stmt=$pdo->prepare("SELECT DISTINCT current_sm_club_id id,current_club name FROM \`$table\` WHERE current_sm_club_id IS NOT NULL ORDER BY current_club");$stmt->execute();$clubs=$stmt->fetchAll();
