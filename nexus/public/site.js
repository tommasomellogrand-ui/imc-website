...
async function teamProfile(c,signal){
 ...
 if(state.profileTab==='transfers'){
  ...
  $('rows').innerHTML=`<article class="card profile-heading">${entity(team,team.name)}</article><div class="transfer-summary">${statTiles([[incoming.length,'Transfer in'],[outgoing.length,'Transfer out'],[all.length,'Movimenti totali']])}</div>${transferTab}<section class="club-transfer-list"><h3>${dir==='in'?'Transfer In':'Transfer Out'}</h3>${active.length?active.map(r=>transferCard(r,dir)).join(''):empty(dir==='in'?'Nessun trasferimento in entrata.':'Nessun trasferimento in uscita.')}</section>`;
