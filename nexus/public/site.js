...
async function fetchData(params,signal){const response=await fetch(`/nexus-next/api.php?${new URLSearchParams(params)}`,{signal,cache:'no-store',credentials:'omit'});const data=await response.json();if(!response.ok||!data.ok)throw new Error('Dati momentaneamente non disponibili. Riprova tra poco.');return data}
