...
function nexus_trophy_editions(array $results,array $schedule,array $catalog,array $seasons): array {
    ...
    foreach(['results'=>$results,'schedule'=>$schedule] as $kind=>$rows)foreach($rows as $r){
        ...
        $season=$matches[0]['imc_season'];$bucket=$key.'::'.$season;$buckets[$bucket]['competition']=$catalogByKey[$key];$buckets[$bucket]['season']=$season;$buckets[$bucket][$kind][(string)$r['sm_fixture_id']]=$r;
    }
    $awards=[];foreach($buckets as $b){$rs=array_values($b['results']??[]);$ss=array_values($b['schedule']??[]);$c=$b['competition'];$a=$c['sm_action']==='league'?nexus_trophy_league($rs,$ss,$c):nexus_trophy_cup($rs,$ss,$c['sm_action']);if(!$a)continue;$a['competition']=$c;$a['season']=$b['season'];$a['id']=$c['competition_key'].'::'.$b['season'];$awards[]=$a;}
...
function nexus_trophies(PDO $db,PDO $core,string $world): array {
    ...
    foreach($awards as &$a){$m=$matches[$index++];$s=$a['side'];$other=$s==='home'?'away':'home';$a['match']=$m;$a['winner']=['name'=>$m[$s.'_core']['name']??$m[$s.'_name'],'core'=>$m[$s.'_core']??null];
        ...
        unset($a['runner_match'],$a['runner_side'],$a['winner_key']);
    }unset($a);
