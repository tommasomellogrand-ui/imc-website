...
foreach($rows as &$r){$national=in_array(strtoupper((string)($r['competition_group']??'')),['NATIONS','NATIONAL'],true);$index=$national?$nationIndex:$clubIndex;foreach(['home','away'] as $side){$id=$r[$side.'_sm_team_id']??$r[$side.'_sm_club_id']??null;$r[$side.'_core']=$id===null?null:($index[(string)$id]??null);$r[$side.'_manager_core']=$managers[(string)($r[$side.'_sm_manager_id']??'')]??null;if($r[$side.'_manager_core']&&empty($r[$side.'_manager_name']))$r[$side.'_manager_name']=$r[$side.'_manager_core']['full_name'];}foreach(['from','to'] as $side)$r[$side.'_core']=$clubIndex[(string)($r[$side.'_sm_world_club_id']??'')]??null;$r['competition_core']=$compIndex[$r['competition_key']??'']??null;$r['country_name']=$countries[(string)($r['sm_country']??'')]??null;$r['player_core']=$players[(string)($r['player_id']??'')]??null;$r['imc_season']=null;$date=$r['match_date']??$r['transfer_date']??null;if($date)foreach($seasons as $s)if($s['imc_season_start_date']&&$date>=$s['imc_season_start_date']&&(!$s['imc_season_end_date']||$date<=$s['imc_season_end_date'])){$r['imc_season']=$s['imc_season'];break;}}unset($r);
