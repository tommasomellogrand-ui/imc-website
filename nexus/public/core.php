...
function nexus_core_enrich(PDO $db,string $world,string $resource,array &$rows): array {
    ...
    $players=[];if($playerIds){$ph=implode(',',array_fill(0,count($playerIds),'?'));foreach(nexus_core_rows($db,"SELECT p.id,p.forename,p.surname,p.image_url,d.nationality,d.position,d.rating,d.age,d.market_value,d.updated_at FROM `IMC Player Codex Global` p LEFT JOIN `IMC Player Codex Global Data` d ON d.player_id=p.id WHERE p.id IN ($ph)",array_values($playerIds)) as $p)$players[(string)$p['id']]=$p;}
    foreach($rows as &$r){
        $national=in_array(strtoupper((string)($r['competition_group']??'')),['NATIONS','NATIONAL'],true);$index=$national?$nationIndex:$clubIndex;
        foreach(['home','away'] as $side){$id=$r[$side.'_sm_team_id']??$r[$side.'_sm_club_id']??null;$r[$side.'_core']=$id===null?null:($index[(string)$id]??null);$r[$side.'_manager_core']=$managers[(string)($r[$side.'_sm_manager_id']??'')]??null;if($r[$side.'_manager_core']&&empty($r[$side.'_manager_name']))$r[$side.'_manager_name']=$r[$side.'_manager_core']['full_name'];}
        foreach(['from','to'] as $side)$r[$side.'_core']=$clubIndex[(string)($r[$side.'_sm_world_club_id']??'')]??null;
        $r['competition_core']=$compIndex[$r['competition_key']??'']??null;$r['country_name']=$countries[(string)($r['sm_country']??'')]??null;$r['player_core']=$players[(string)($r['player_id']??'')]??null;
        $r['imc_season']=null;$date=$r['match_date']??$r['transfer_date']??null;if($date)foreach($seasons as $s)if($s['imc_season_start_date']&&$date>=$s['imc_season_start_date']&&(!$s['imc_season_end_date']||$date<=$s['imc_season_end_date'])){$r['imc_season']=$s['imc_season'];break;}
    }unset($r);
...
function nexus_world_directory(PDO $db,string $world): array {
    ...
    $context['clubs']=nexus_core_rows($db,"SELECT m.`Club ID` id,COALESCE(c.name,m.`Club Name`) name,c.image_url,m.`SM Club ID` sm_team_id,a.manager_id,mc.full_name manager_name,a.start_date manager_start_date FROM `IMC Game World Club Mapping` m LEFT JOIN `IMC Club Codex Global` c ON c.id=m.`Club ID` LEFT JOIN `IMC Manager Assignment Global` a ON a.game_world_id=m.`Game World` AND a.team_id=m.`Club ID` AND (a.end_date IS NULL OR a.end_date>=CURRENT_DATE) LEFT JOIN `IMC Manager Codex Global` mc ON mc.manager_id=a.manager_id WHERE m.`Game World`=? ORDER BY name",[$world]);
    $context['managers']=nexus_core_rows($db,'SELECT a.manager_id,a.team_id,a.national_team_id,COALESCE(m.full_name,a.full_name) full_name,a.team_name,a.assignment_type,a.start_date,a.end_date FROM `IMC Manager Assignment Global` a LEFT JOIN `IMC Manager Codex Global` m ON m.manager_id=a.manager_id WHERE a.game_world_id=? ORDER BY full_name,a.start_date DESC',[$world]);
    ...
    $context['nations']=nexus_core_rows($db,'SELECT m.`National Team ID` id,c.name,c.image_url,m.`SM National Team ID` sm_team_id FROM `IMC Game World National Team Mapping` m LEFT JOIN `IMC National Team Codex Global` c ON c.id=m.`National Team ID` WHERE m.`Game World`=? ORDER BY c.name',[$world]);
    return $context;
}
