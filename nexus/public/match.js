...
async function fetchJSON(query){const response=await fetch('/nexus/api.php?'+new URLSearchParams(query),{cache:'no-store',credentials:'omit'});const d=await response.json();if(!response.ok||!d.ok)throw Error('Il report non è momentaneamente disponibile.');return d}
async function boot(){
