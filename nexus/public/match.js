...
const fixture=params.get('fixture')||'',tabs=[['overview','Riepilogo'],['lineup','Formazioni'],['ratings','Voti'],['events','Eventi']];
...
function playerTable(lineup=false){const list=players.filter(p=>p.team_side===side).sort((a,b)=>(num(a.squad_slot)??999)-(num(b.squad_slot)??999)),starters=list.filter(p=>flag(p.starter)),subs=list.filter(p=>!flag(p.starter));return `<section class="box"><div class="section-head"><h2>${lineup?'Formazione':'Voti e prestazioni'}</h2>${switcher()}</div>${list.length?`<div class="rating-head"><span>#</span><span>GIOCATORE</span><span>MIN</span><span>VOTO</span><span>EVENTI</span></div>${starters.map(playerRow).join('')}${subs.length?'<div class="bench">Panchina e sostituti</div>'+subs.map(playerRow).join(''):''}<p class="note">— indica un dato non disponibile.</p>`:empty('Formazione non disponibile per questa squadra.')}</section>`}
...
function render(){stateURL();$('tabs').hidden=false;$('tabs').innerHTML=tabs.map(([key,label])=>`<a data-view href="${esc(href({tab:key}))}" ${tab===key?'aria-current="page"':''}>${label}</a>`).join('');$('panel').innerHTML=tab==='ratings'?playerTable():tab==='lineup'?formation():tab==='events'?eventView():overview()}
