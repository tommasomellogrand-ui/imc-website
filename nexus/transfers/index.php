...
try{
 ...
 $playerImages=[];$ids=array_values(array_unique(array_filter(array_map(fn($x)=>(int)($x['player_id']??0),$rows))));if($ids){$ph=implode(',',array_fill(0,count($ids),'?'));$players=nexus_rows($core,'SELECT id,image_url FROM `IMC Player Codex Global` WHERE id IN ('.$ph.')',$ids);foreach($players as $x)$playerImages[(string)$x['id']]=$x['image_url']??null;}
