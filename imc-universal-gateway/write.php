...
if(in_array($a,['insert','upsert'],true)){$outcome=imc_schedule_import($p,$t,true,fn()=>imc_insert($p,$t,$b['row']??[],$a==='upsert'));$n=$outcome['written'];imc_log($r,$a,$t,['affected_rows'=>$n]);imc_out($B+['normalization'=>$outcome['normalization'],'inserted_rows'=>$n,'last_insert_id'=>$p->lastInsertId()]);}
if(in_array($a,['insert_many','upsert_many'],true)){$rows=$b['rows']??[];if(!is_array($rows)||!$rows)throw new InvalidArgumentException('required_field_missing');$atomic=($b['atomic']??true)!==false;$outcome=imc_schedule_import($p,$t,$atomic,function()use($rows,$p,$t,$a){$n=0;foreach($rows as$row)$n+=imc_insert($p,$t,$row,$a==='upsert_many');return $n;});$n=$outcome['written'];imc_log($r,$a,$t,['affected_rows'=>$n]);imc_out($B+['normalization'=>$outcome['normalization'],'received'=>count($rows),'inserted'=>$n,'written_rows'=>$n,'skipped_rows'=>0,'failed'=>0,'rolled_back'=>false,'errors'=>[]]);}
if($a==='update'){$set=imc_row($p,$t,$b['set']??[]);if(!$set)throw new InvalidArgumentException('required_field_missing');$v=[];$q=[];foreach($set as$k=>$x){$q[]=imc_ident($k).'=?';$v[]=$x;}$w=$b['where']??[];$before=imc_count($p,$t,$w);$sql='UPDATE '.imc_ident($t).' SET '.implode(',',$q).' WHERE '.imc_where($w,$v);$s=$p->prepare($sql);$outcome=imc_schedule_import($p,$t,true,function()use($s,$v){$s->execute($v);return $s->rowCount();});imc_log($r,$a,$t,['affected_rows'=>$outcome['written']]);imc_out($B+['normalization'=>$outcome['normalization'],'matched_rows'=>$before,'updated_rows'=>$outcome['written']]);}
