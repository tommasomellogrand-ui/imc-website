...
function imc_transfer_table(string $t):bool{return preg_match('/^GW00[1-9]_IMC Transfers$/',$t)===1;}
...
if(in_array($a,['insert_many','upsert_many'],true)){$rows=$b['rows']??[];if(!is_array($rows)||!$rows)throw new InvalidArgumentException('required_field_missing');$atomic=($b['atomic']??true)!==false;$n=0;if($atomic)$p->beginTransaction();try{foreach($rows as$row)$n+=imc_insert($p,$t,$row,$a==='upsert_many');if($atomic)$p->commit();}catch(Throwable$e){if($p->inTransaction())$p->rollBack();throw$e;}imc_log($r,$a,$t,['affected_rows'=>$n]);imc_out($B+['received'=>count($rows),'inserted'=>$n,'written_rows'=>$n,'skipped_rows'=>0,'failed'=>0,'rolled_back'=>false,'errors'=>[]]);}
