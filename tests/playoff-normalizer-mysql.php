...
try {
    ...
    foreach(['Results','Match Report'] as $repo) $db->exec("CREATE TABLE `GW001_IMC {$repo}` (game_world_id VARCHAR(16),sm_fixture_id BIGINT,sm_action VARCHAR(64),sm_country VARCHAR(128),sm_division VARCHAR(128),competition_group VARCHAR(64),competition_key VARCHAR(255),competition_stage VARCHAR(128),competition_round VARCHAR(128),match_date DATE,home_sm_club_id BIGINT,away_sm_club_id BIGINT,home_score INT,fingerprint CHAR(64)) ENGINE=InnoDB");
    $insert=function(string $repo,int $id,string $action,string $stage,string $date,string $division)use($db):int{
        $s=$db->prepare("INSERT INTO `GW001_IMC {$repo}` (game_world_id,sm_fixture_id,sm_action,sm_division,competition_group,competition_key,competition_stage,match_date,home_sm_club_id,away_sm_club_id,home_score,fingerprint) VALUES ('GW001',?,?,?,'DOMESTIC','GW001|DOMESTIC|league|4',?,?,11,22,3,'original-import-fingerprint')");
        $s->execute([$id,$action,$division,$stage,$date]);return $s->rowCount();
    };
    $first=imc_playoff_import($db,'GW001_IMC Results',true,function()use($insert){return $insert('Results',100,'playoff','Playoff Finale','2026-09-14','4')+$insert('Match Report',100,'league','Playoff Finale','2026-09-14','4');});
    verify($first['normalization']['unresolved']===2,'out-of-order pending');
    verify($db->query('SELECT competition_key FROM `GW001_IMC Results`')->fetchColumn()===null,'guessed key cleared');
    $next=imc_playoff_import($db,'GW001_IMC Results',true,fn()=>$insert('Results',99,'league','Giornata 18','2026-09-10','2'));
    ...
    foreach(['Results','Match Report'] as $repo){
        $r=$db->query("SELECT * FROM `GW001_IMC {$repo}` WHERE sm_fixture_id=100")->fetch();
    ...
    $again=imc_playoff_import($db,'GW001_IMC Results',true,fn()=>0);
    verify($again['normalization']['updated']===0,'idempotent');
    try {imc_playoff_import($db,'GW001_IMC Results',true,function()use($insert){$insert('Results',101,'league','','2026-09-10','2');throw new RuntimeException('test rollback');});}catch(RuntimeException $e){verify($e->getMessage()==='test rollback','expected error');}
    verify((int)$db->query('SELECT COUNT(*) FROM `GW001_IMC Results` WHERE sm_fixture_id=101')->fetchColumn()===0,'failed batch rollback');
    ...
    try {imc_playoff_import($db,'GW001_IMC Results',true,fn()=>$insert('Results',102,'league','','2026-09-10','2'));throw new RuntimeException('missing CORE accepted');}catch(PDOException $e){}
    verify((int)$db->query('SELECT COUNT(*) FROM `GW001_IMC Results` WHERE sm_fixture_id=102')->fetchColumn()===0,'CORE failure rolls back import');
