...
try {
 ...
 $db->exec('CREATE TABLE `GW010_IMC Schedule` '.$schema);$db->exec('CREATE TABLE `IMC Site Schedule` '.$schema);
 $db->exec('CREATE TRIGGER schedule_test_ai AFTER INSERT ON `GW010_IMC Schedule` FOR EACH ROW INSERT INTO `IMC Site Schedule` VALUES (NEW.game_world_id,NEW.sm_fixture_id,NEW.match_date,NEW.home_name,NEW.away_name,NEW.home_sm_team_id,NEW.away_sm_team_id,NEW.logged_manager_id,NEW.fingerprint,NEW.competition_group,NEW.sm_action)');
 ...
 $out=imc_ingress_insert_many($db,'GW010_IMC Schedule','GW010',[$row],true);
 verify_schedule($out['written_rows']===1 && $out['normalization']['updated_ids']===1,'ingress import resolves missing ID');
 foreach(['GW010_IMC Schedule','IMC Site Schedule'] as $table){$r=$db->query("SELECT * FROM `{$table}` WHERE sm_fixture_id=100")->fetch();verify_schedule((int)$r['home_sm_team_id']===92317315 && $r['fingerprint']==='original','source and mirror consistent; fingerprint unchanged');}
 $out=imc_schedule_import($db,'GW010_IMC Schedule',true,fn()=>0);verify_schedule($out['normalization']['updated_ids']===0,'idempotent');
 $out=imc_ingress_insert_many($db,'GW010_IMC Schedule','GW010',[array_replace($row,['sm_fixture_id'=>101,'home_sm_team_id'=>55]),array_replace($row,['sm_fixture_id'=>102,'home_name'=>'Unmatched'])],true);
 verify_schedule((int)$db->query('SELECT home_sm_team_id FROM `GW010_IMC Schedule` WHERE sm_fixture_id=101')->fetchColumn()===55,'existing ID preserved');
 ...
 try {imc_ingress_insert_many($db,'GW010_IMC Schedule','GW010',[array_replace($row,['sm_fixture_id'=>103])],true);throw new RuntimeException('expected failure');}catch(PDOException $e){}
 verify_schedule((int)$db->query('SELECT COUNT(*) FROM `GW010_IMC Schedule` WHERE sm_fixture_id=103')->fetchColumn()===0,'core failure rolls back source');
 verify_schedule((int)$db->query('SELECT COUNT(*) FROM `IMC Site Schedule` WHERE sm_fixture_id=103')->fetchColumn()===0,'core failure rolls back mirror');
