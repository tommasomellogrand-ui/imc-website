...
export function renderCompetitionHub(h,{esc,link,head,empty,source,score,schedule,teamLink,date}){
  ...
  const trophy=h.winner?`<article class="record" data-winner="${esc(h.winner.name)}"><span class="eyebrow">VINCITORE · FINALE</span><h3 class="serif" style="font-size:2rem;margin:18px 0">${teamLink(h.winner.identity,h.winner.name)}</h3><p>Finale del ${date(h.winner.fixture.match_date)}</p>${score(h.winner.fixture)}</article>`:
  ...
  const contents={results:`<p class="source">${h.results.length} risultati · tutte le fasi della competizione.</p><div class="match-list">${h.results.map(r=>`<div data-hub-fixture-key="${esc(r.competition_key)}">${phase(r)?`<p class="source">${esc(phase(r))}</p>`:''}${score(r)}</div>`).join('')||empty('Nessun risultato disponibile per questa competizione.')}</div>`,
    schedule:`<p class="source">Incontri da oggi · Europe/Rome · tutte le fasi della competizione.</p>`+(h.schedule.length?h.schedule.map(r=>`<div data-hub-schedule-key="${esc(r.competition_key)}">${phase(r)?`<p class="source">${esc(phase(r))}</p>`:''}${schedule([r])}</div>`).join(''):schedule([])),standings:ranking,trophy};
