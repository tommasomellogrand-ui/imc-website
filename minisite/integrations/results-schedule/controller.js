import { readFixtures, selectFixtures, countries, validDay } from './adapter.js';
...
export function mountFixtures({gw,resource,render,esc}) {
  let data=null,controller=null,loading=false,generation=0;
  ...
  function draw(){
    ...
    render(records);count.textContent=`${records.length} ${resource==='results'?'risultati':'incontri'}`;
  ...
  async function load(){
    controller?.abort();controller=new AbortController();const current=++generation;loading=true;controls.forEach(c=>c.disabled=true);render([]);count.textContent='';setStatus('loading','Caricamento in corso…');
    const timeout=setTimeout(()=>controller.abort(),45000);
    try {const next=await readFixtures(gw,resource,{signal:controller.signal});if(current!==generation)return;data=next;draw();}
    catch(error){if(current!==generation)return;data=null;render([]);setStatus(error.message==='source_changed'?'stale':'error',error.message==='source_changed'?'La fonte è cambiata durante la lettura. Riprova per ottenere dati coerenti.':'Lettura non disponibile. Nessun dato sostitutivo viene mostrato.');const retry=document.createElement('button');retry.type='button';retry.textContent='Riprova';retry.addEventListener('click',load);status.append(' ',retry);}
