...
function smm_handle_ingestion():never{
    ...
    if(in_array($type,['results','schedule','match_report','player_codex','transfers'],true)){
...
function smm_ingestion_auth(array $payload=[]):void{
    ...
    $existingImporterAuthorized=$token!==''&&in_array($importerType,['results','schedule','match_report','transfers'],true)&&hash_equals(SMM_MATCH_REPORT_INGESTION_TOKEN,$token);
